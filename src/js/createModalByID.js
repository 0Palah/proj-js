...
export const createModalByID = async event => {
    ...
    try {
        ...
        const { data: { page: { totalElements }, _embedded: { events } } } = await developerApi.fetchDataByZId();
        console.log('\ntotalElements:', totalElements);
        console.log('events:', events);
        console.log('\n');

        let { dates: { start: { localDate, localTime }, timezone }, images, name, _embedded: { venues } } = events[0];
